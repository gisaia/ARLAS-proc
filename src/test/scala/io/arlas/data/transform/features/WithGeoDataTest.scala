...
class WithGeoDataTest extends ArlasTest with ArlasMockServer {
  ...
  "WithGeoData" should "get GeoData" in {
    ...
    val transformedDF = baseDF.process(new WithGeoData("http://localhost:8080/reverse.php?format=json&lat=%2f&lon=%2f&zoom=%s","lat", "lon", addressColumnsPrefix))
  ...
  "WithGeoData" should "get GeoData when a 'null' condition is true" in {
    ...
    val transformedDF =
      baseDF
        ...
        .process(new WithGeoData("http://localhost:8080/reverse.php?format=json&lat=%2f&lon=%2f&zoom=%s","lat", "lon", addressColumnsPrefix, Some("do_get_address")))
  ...
  "WithGeoData" should "not stringify 0.000932 to 9.32E^4 and fail with bad coordinates" in {
    ...
    val transformedDF = expectedDF
      .drop("expected_city", "expected_county", "expected_state", "expected_country", "expected_country_code", "expected_postcode")
      .process(new WithGeoData("http://localhost:8080/reverse.php?format=json&lat=%2f&lon=%2f&zoom=%s", "lat", "lon", "expected_"))
