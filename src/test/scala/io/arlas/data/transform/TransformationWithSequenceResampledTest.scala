      ...
      row =>
        row._1.equals("ObjectA")
          && LocalDateTime
            ...
            .isBefore(
              LocalDateTime.parse("01/06/2018 00:10:00+02:00", timeFormatter)))
      ...
      row =>
        row._1.equals("ObjectB")
          && LocalDateTime
            .parse(row._2, timeFormatter)
            .isBefore(
              LocalDateTime.parse("01/06/2018 00:07:00+02:00", timeFormatter)))
      ...
      row =>
        row._1.equals("ObjectB")
          && LocalDateTime
            ...
            .isAfter(
              LocalDateTime.parse("01/06/2018 00:07:00+02:00", timeFormatter)))
    expectedInterpolation(objectASeq1Raw, 15) ++ expectedInterpolation(
      objectASeq2Raw,
      15) ++ expectedInterpolation(objectBSeq1Raw, 15) ++ expectedInterpolation(
      objectBSeq2Raw,
      15)
  }

  def expectedInterpolation(
      data: Seq[(String, String, Double, Double)],
      timeSampling: Long): Seq[(String, String, Double, Double, String)] = {
    val dataTimestamped = data
      .map(
        row =>
          (ZonedDateTime.parse(row._2, timeFormatter).toEpochSecond(),
           row._3,
           row._4))
      .distinct
      .sortBy(_._1)
