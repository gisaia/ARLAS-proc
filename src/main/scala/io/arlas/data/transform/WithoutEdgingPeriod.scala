...
class WithoutEdgingPeriod(dataModel: DataModel,
  ...
  override def transform(dataset: Dataset[_]): DataFrame = {
    ...
    val filterByWarming = dataset
      .toDF()
      .map(row =>
        (row.getString(row.fieldIndex(arlasSequenceIdColumn)),
  ...
  def filterEdgingPeriod(dataModel: DataModel,
    ...
    val orderedRawSequence = rawSequence.sortBy(row =>
      row.getOrElse(arlasTimestampColumn, 0l).asInstanceOf[Long])
